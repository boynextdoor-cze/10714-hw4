...
class ReLU(TensorOp):
    ...
    def gradient(self, out_grad, node):
        # BEGIN YOUR SOLUTION
        return out_grad * Tensor(node.inputs[0].cached_data > 0, device=node.inputs[0].device)
        # END YOUR SOLUTION
...
class Split(TensorTupleOp):
    ...
    def compute(self, A):
        ...
        for i in range(input_shape[self.axis]):
            ...
            slices[self.axis] = i
            output.append(A[tuple(slices)].compact().reshape(output_shape))
        return tuple(output)
        ### END YOUR SOLUTION
...
class Conv(TensorOp):
    ...
    def compute(self, A, B):
        ...
        out_H = (H - KH + self.stride) // self.stride
        out_W = (W - KW + self.stride) // self.stride

        inner_dim = KH * KW * C_in
        A = A.as_strided(shape=(N, out_H, out_W, KH, KW, C_in), strides=(Ns, Hs * self.stride, Ws * self.stride, Hs, Ws, Cs)).compact()
        A = A.reshape((N * out_H * out_W, inner_dim)).compact()
        out = A @ B.reshape((inner_dim, C_out)).compact()
        return out.reshape((N, out_H, out_W, C_out)).compact()
        ### END YOUR SOLUTION
